...
final class Provider
{
     ...
     * @var string|null
     */
    private $signedTransaction = null;
     ...
     */
    public function __construct($network, bool $testnet, int $timeOut = 5)
    {
        $networks = $testnet ? EthBasedNetworks::$testnets : EthBasedNetworks::$mainnets;
    ...
    public function tokenTransfer(string $from, string $to, float $amount, string $tokenAddress) : Provider
    {
        ...
        $this->pendingTransaction = (new Token($tokenAddress))->transfer($from, $to, $amount);
    ...
    public function coinTransfer(string $from, string $to, float $amount) : Provider
    {
        ...
        $this->pendingTransaction = (new Coin())->transfer($from, $to, $amount);
    ...
    public function sign(string $privateKey, ?PendingTransaction $pendingTransaction = null) : string
    {
        ...
        if ($this->pendingTransaction instanceof PendingTransaction) {
            return $this->signedTransaction = $this->pendingTransaction->sign($privateKey);
        } else {
